...
export function OneLinerView({ campaignId, campaignName, wfNumber, shoots }: Props) {
  ...
  const addShootDay = async () => {
    ...
    try {
      ...
      // Refetch to reconcile the temp row with the server's authoritative
      // state. The optimistic paint already made the new column visible,
      // so this happens in the background without UI lag.
