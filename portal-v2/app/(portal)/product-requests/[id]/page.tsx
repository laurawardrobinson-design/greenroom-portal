...
import type {
  ...
  PRDepartment,
  PRDocStatus,
  PR_DEPARTMENT_LABELS,
} from "@/types/domain";
import { PR_DEPARTMENTS, PR_STATUS_LABELS } from "@/types/domain";
...
// --- Item row (editable or read-only) ---
function ItemRow({
  item,
  editable,
  onUpdate,
    ...
    <tr className="group border-t border-border/50">
      ...
      <td className="py-2 px-2">
        <div className="flex items-center gap-1.5">
          ...
          {item.fromShotList && (
            <span className="text-[10px] bg-sky-50 text-sky-600 rounded px-1 py-0.5 shrink-0">
              Shot list
            </span>
          )}
        </div>
  ...
  const updateSection = useCallback(
    async (field: "dateNeeded" | "timeNeeded" | "pickupPerson", value: string) => {
      await fetch(`/api/product-requests/${docId}/sections`, {
        ...
        body: JSON.stringify({ department: section.department, [field === "dateNeeded" ? "dateNeeded" : field === "timeNeeded" ? "timeNeeded" : "pickupPerson"]: value }),
      });
      onRefresh();
    },
