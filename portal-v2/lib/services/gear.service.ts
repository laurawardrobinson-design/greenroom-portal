...
function toGearItem(row: Record<string, unknown>): GearItem {
  return {
    ...
    section: (row.section as "Gear" | "Props") || "Gear",
...
export async function getActiveCheckouts(userId?: string): Promise<GearCheckout[]> {
  ...
  return (data || []).map((row) => ({
    ...
    user: row.users
      ? { id: row.users.id, email: row.users.email, name: row.users.name, role: row.users.role, active: row.users.active, avatarUrl: row.users.avatar_url || "", phone: row.users.phone || "", title: row.users.title || "", vendorId: row.users.vendor_id, deskDepartment: row.users.desk_department ?? null, favoriteDrinks: row.users.favorite_drinks || "", favoriteSnacks: row.users.favorite_snacks || "", dietaryRestrictions: row.users.dietary_restrictions || "", allergies: row.users.allergies || "", energyBoost: row.users.energy_boost || "", favoritePublixProduct: row.users.favorite_publix_product || "", lunchPlace: row.users.lunch_place || "", preferredContact: row.users.preferred_contact || "Email", onboardingCompleted: row.users.onboarding_completed ?? false, createdAt: row.users.created_at, updatedAt: row.users.updated_at }
      : undefined,
  }));
}
